// surface.go defines the two Claude Code CLI 2.1.206 surface
// profiles the plugin can impersonate. Each SurfaceProfile captures the
// per-entrypoint fingerprint tuple (agent identity, anthropic-beta set, and
// system[3] static text) extracted from real mitmproxy captures.
//
// The captures live under testdata/captures/ and are re-derived by
// scripts/recon/*. The shared intro/security/tone bundle (system[2]) and the
// static prefix of system[3] ("# Text output ...") are byte-identical between
// the cli (interactive TUI) and sdk-cli (-p print) surfaces, so they are stored
// once via //go:embed and referenced by both profiles.
...
type SurfaceProfile struct {
	...
	Betas []string
	// TextOutputSection is the STATIC prefix of system[3] (the "# Text output"
	// section up to but not including the client-dynamic
	// "# Session-specific guidance" tail). The plugin never emits the dynamic
	// tail — that belongs to the real client.
	TextOutputSection string
}

// sharedSystemIntro is the surface-invariant system[2] block (the intro +
// security + System + DoingTasks + Tone bundle) captured from CLI 2.1.206. It
// is byte-identical between cli and sdk-cli surfaces.
//
//go:embed surfacedata/shared_intro.txt
var sharedSystemIntro string

// sharedTextOutputSection is the STATIC prefix of system[3] ("# Text output ..."
// through the paragraph immediately before "# Session-specific guidance"). It
// is byte-identical between the two surfaces in the 2.1.206 captures, so both
// profiles reference this shared const. Keeping the field per-profile in
// SurfaceProfile still lets a future capture diverge cleanly.
//
//go:embed surfacedata/text_output.txt
var sharedTextOutputSection string
...
var CLISurface = SurfaceProfile{
	...
	Betas:             cliBetas,
	TextOutputSection: sharedTextOutputSection,
}
