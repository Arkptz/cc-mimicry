...
## Context
cc-mimicry is a CLIProxyAPI plugin compiled as a CGO c-shared shared object (.so). The previous build required a multi-repo Docker context (cc-mimicry + CLIProxyAPI side by side) plus a builder-service and named-volume hack in the dev compose stack — fragile, non-reproducible, and it blocked independent plugin CI. The plugin also requires Go 1.26.1 and CGO, which conflicts with the Go 1.24.13 toolchain standardized in ADR-001.
## Decision
...
4. Go 1.26 toolchain pin: the go.mod `go 1.26.1` directive + Nix `go_1_26` in the dev shell/CI + `GOTOOLCHAIN=local` (forbids silent toolchain downloads). A redundant `toolchain go1.26.1` directive is deliberately NOT used — Go strips a toolchain directive equal to the go directive on `go mod tidy` and then refuses to build until it is gone (see POL-002). This supersedes ADR-001's Go 1.24.13 standardization.
## Consequences
### Positive
